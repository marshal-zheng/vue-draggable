...
export function addEvent (el: Node, event: string, handler: EventListenerOrEventListenerObject, inputOptions?: AddEventListenerOptions): void {
  if (!el) return
  const options = {capture: true, ...inputOptions}
  if (el.addEventListener) {
    el.addEventListener(event, handler as EventListener, options)
  } else if ((el as any).attachEvent) {
    (el as any).attachEvent(`on${event}`, handler as EventListener)
  } else {
    (el as any)[`on${event}`] = handler
  }
}

export function removeEvent (el: Node, event: string, handler: EventListenerOrEventListenerObject, inputOptions?: AddEventListenerOptions): void {
  if (!el) return
  const options = {capture: true, ...inputOptions}
  if (el.removeEventListener) {
    el.removeEventListener(event, handler as EventListener, options)
  } else if ((el as any).detachEvent) {
    (el as any).detachEvent('on' + event, handler as EventListener)
  } else {
...
export function createCSSTransform (controlPos: ControlPosition, positionOffset: PositionOffsetControlPosition): unknown {
